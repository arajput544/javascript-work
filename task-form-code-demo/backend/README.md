...
1. Edit the config.json file to match your Mongo Atlas Cloud details. Edit the URL in the server.js to match your Mongo Atlas Cloud details.
...
3. Once you have chosen and completed any associated requirements for Option 1 or Option 2. Import the task-app-test.json into POSTMAN to view the URLs and POST/PUT body for each endpoint. When making requests - REPLACE ALL identifiers (ids) with actual ids from your Mongo Cloud Atlas Database. This includes ids in the URLs, ids in the POST/PUT body, etc. ALL the ids should be ids from your Mongo Cloud Atlas Databse.
